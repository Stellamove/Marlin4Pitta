...
// PITTA
#if HAS_PITTA_MMU
#define PITTA_TIMER_IRQ_PRIO_DEFAULT     2
#endif
...
// PITTA
#if HAS_PITTA_MMU
  #ifndef HAL_PITTA_TIMER_RATE
    #define HAL_PITTA_TIMER_RATE GetPittaTimerClkFreq()
  #endif
  #ifndef PITTA_TIMER
    #define PITTA_TIMER MCU_PITTA_TIMER
  #endif
#endif
...
// PITTA
#if HAS_PITTA_MMU
uint32_t GetPittaTimerClkFreq() {
  // Timer input clocks vary between devices, and in some cases between timers on the same device.
  // Retrieve at runtime to ensure device compatibility. Cache result to avoid repeated overhead.
  static uint32_t clkfreq = timer_instance[MF_TIMER_PITTA]->getTimerClkFreq();
  return clkfreq;
}
#endif
...
void HAL_timer_start(const uint8_t timer_num, const uint32_t frequency) {
  if (!HAL_timer_initialized(timer_num)) {
    switch (timer_num) {
      ...
      // PITTA
      #if HAS_PITTA_MMU
        case MF_TIMER_PITTA:  // PITTA TIMER - any available 16bit timer
          timer_instance[timer_num] = new HardwareTimer(PITTA_TIMER_DEV);
          timer_instance[timer_num]->setPrescaleFactor(PITTA_TIMER_PRESCALE);
          timer_instance[timer_num]->setOverflow(_MIN(hal_timer_t(HAL_TIMER_TYPE_MAX), (HAL_PITTA_TIMER_RATE) / (PITTA_TIMER_PRESCALE)), TICK_FORMAT);
          break;
      #endif
    }
    ...
    switch (timer_num) {
      ...
      // PITTA
      case MF_TIMER_PITTA:
        timer_instance[timer_num]->setInterruptPriority(PITTA_TIMER_IRQ_PRIO, 0);
        break;
    }
  }
}
...
void HAL_timer_enable_interrupt(const uint8_t timer_num) {
  if (HAL_timer_initialized(timer_num) && !timer_instance[timer_num]->hasInterrupt()) {
    switch (timer_num) {
      ...
      // PITTA
      case MF_TIMER_PITTA:
        timer_instance[timer_num]->attachInterrupt(Pitta_Handler);
        break;
    }
  }
}
...
// PITTA
#if HAS_PITTA_MMU
enum TimerPurpose { TP_SERIAL, TP_TONE, TP_SERVO, TP_STEP, TP_TEMP, TP_PITTA };
#else
