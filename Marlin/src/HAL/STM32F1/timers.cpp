...
void HAL_timer_start(const uint8_t timer_num, const uint32_t frequency) {
  ...
  switch (timer_num) {
    ...
    #if HAS_PITTA_MMU
      case MF_TIMER_PITTA:
        timer_pause(PITTA_TIMER_DEV);
        timer_set_count(PITTA_TIMER_DEV, 0);
        timer_set_prescaler(PITTA_TIMER_DEV, (uint16_t)(PITTA_TIMER_PRESCALE - 1));
        timer_set_reload(PITTA_TIMER_DEV, 0xFFFF);
        timer_generate_update(TEMP_TIMER_DEV);
        timer_resume(PITTA_TIMER_DEV);
        break;
    #endif
  }
}
